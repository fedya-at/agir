...
const Analytics = () => {
  ...
  const userRolesCount = users.reduce(
    (acc, user) => {
  ...
  const partsLowStock = parts.filter((p) => p.quantity <= 5);
  const invoicesPaid = invoices.filter((inv) => inv.paid).length;
  const invoicesUnpaid = invoices.length - invoicesPaid;
