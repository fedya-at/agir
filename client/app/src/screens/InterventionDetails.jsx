...
const InterventionDetails = () => {
  ...
  useEffect(() => {
    ...
    // Allow Admin, Client, and Technician roles to check for invoices
    if (
    ...
    >
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <Box sx={{ display: "flex", flexDirection: "row", gap: 4 }}>
          <Box sx={{ flex: 2 }}>
            <InterventionDetailsSection intervention={currentIntervention} />
            <Box>
              <PartsSection
                intervention={currentIntervention}
                isTechnician={isTechnician}
            ...
            {(isAdmin || isTechnician) && currentIntervention.status === 2 && (
              <Box>
                <InvoiceSection
                  ...
                  onGenerateClick={handleGenerateInvoice} // Pass the handler directly
                />
              </Box>
            )}
            {isTechnician && (
              <Box sx={{ mt: 3 }}>
                <ClientInfoCard
                  key={currentIntervention.client.id} // Add key to prevent remounts
