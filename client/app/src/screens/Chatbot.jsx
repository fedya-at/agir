...
const RAG_API_BASE_URL = "http://127.0.0.1:5001/api/rag_query";
const HEALTH_API_URL = "http://127.0.0.1:5001/health";
const MODELS_API_URL = "http://127.0.0.1:5001/api/models";
const CLEAR_MEMORY_API_URL = "http://127.0.0.1:5001/api/clear_memory";
...
const Chatbot = () => {
  ...
  const { messages, isLoading, error, lastIntent, confidence } = useSelector(
    (state) => state.chat
  );
  ...
  const checkHealthStatus = async () => {
    try {
      ...
      if (health.status === "healthy") {
        toast.success("System is healthy", { duration: 2000 });
      } else if (health.status === "initializing") {
        toast.loading("System is initializing...", { duration: 2000 });
      } else {
        toast.error("System health check failed", { duration: 3000 });
      }
    } catch (error) {
      ...
      toast.error("Failed to fetch health status", { duration: 3000 });
    }
  };
  ...
  const fetchAvailableModels = async () => {
    try {
      ...
      toast.error("Failed to fetch available models", { duration: 3000 });
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    if (healthStatus && !healthStatus.rag_initialized) {
      toast.error(
        "RAG system is not initialized. Please check the server status.",
        {
          duration: 5000,
        }
      );
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        dispatch(
          addBotResponse({
            response:
              "I'm receiving too many requests right now. Please wait a moment before asking another question.",
            type: "rate_limit",
            id: botMessageId,
          })
        );
      ...
      let errorMessage = "Connection error with RAG server.";
      if (error.message.includes("Failed to fetch")) {
        errorMessage =
          "Cannot connect to the server. Please ensure the Flask server is running.";
      ...
      dispatch(
        addBotResponse({
          response: `Sorry, I cannot connect to the RAG server right now. ${errorMessage}`,
          type: "error",
          id: botMessageId,
        })
      );
        ...
        <Container maxWidth="lg">
          ...
          <Box sx={{ textAlign: "center", mb: 6 }}>
            ...
            {lastIntent && confidence && (
              <Box sx={{ mt: 2 }}>
                <Chip
                  label={`Intent: ${lastIntent} (${(confidence * 100).toFixed(
                    1
                  )}%)`}
                  size="small"
                  sx={{
