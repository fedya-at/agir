...
export const fetchInterventionsByTechnicianId = createAsyncThunk(
  ...
  async (technicianId, { rejectWithValue }) => {
    try {
      ...
      console.log("Fetched Interventions from API:", response.data); // Debug log
      return response.data;
    } catch (error) {
      console.error("Error fetching interventions:", error); // Debug log
      return rejectWithValue(error.response.data);
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchInterventionsByTechnicianId.fulfilled, (state, action) => {
        console.log(
          "Updating interventionsByTechnician state:",
          action.payload
        ); // Debug log
